...
namespace D004.同步設計披薩製作
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            tasks.Add(Task.Run(() => 烤箱預熱()));
            tasks.Add(Task.Run(() => 製作麵團()));
            await Task.WhenAll(tasks);
            tasks.Clear();

            tasks.Add(Task.Run(() => 發麵()));
            tasks.Add(Task.Run(() => 準備醬料()));
            tasks.Add(Task.Run(() => 準備配料()));
            ...
            tasks.Clear();
            tasks.Add(Task.Run(() => 烤製披薩()));
            tasks.Add(Task.Run(() => 準備餐具與飲料()));
            await Task.WhenAll(tasks).ContinueWith((t)=>
            {
                披薩完成_開始食用();
            });
            ...
            watch.Stop();
            Console.WriteLine($"非同步設計披薩製作共花費:{watch.Elapsed.TotalSeconds} 秒");
